...
function applyDarkMode(isDarkMode) {
  if (isDarkMode) {
    ...
    document
      .querySelectorAll(".list-group-item, .comment, .card")
      .forEach((element) => {
...
// Función para mostrar la información del producto y agregar el botón "Comprar"
function showProductInfo(product) {
  const isDarkMode =
    localStorage.getItem(`${loggedInUser}_darkMode`) === "true"; // Asegurarnos que sea por usuario
          ...
          isDarkMode ? "bg-dark text-light" : "bg-white text-dark"
            ...
            <div class="row">
                <div class="col-12 col-md-6">
                    <h3>${product.name}</h3>
                    <hr>
                    <p><strong>Precio:</strong> ${product.currency} ${
    product.cost
  }</p>
                    <p><strong>Descripción:</strong> ${product.description}</p>
                    <p><strong>Categoría:</strong> ${product.category}</p>
                    <p><strong>Cantidad vendidos:</strong> ${
                      product.soldCount
                    }</p>
                </div>
                <div class="col-12 col-md-6">
  ...
  buyButton.className = "btn btn-primary mt-3";
          ...
          .map(
            (comment) =>
              `<div class="comment ${
                ...
                <p><strong>${comment.user}</strong> - ${formatDate(
                new Date(comment.dateTime)
              )}</p>
                <p>${"★".repeat(comment.score)}${"☆".repeat(
                5 - comment.score
              )}</p>
